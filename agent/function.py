async def original_agentless_method():
    ...
    problem_statement = await github_issue_client.get_issue_description(
        owner="OpenBMB", repo="RepoAgent", issue_number=70
    )

    prompt = qa_template.format(
        problem_statement=problem_statement, structure=tree_string
    )
